...
Element.Methods = {
    ...
    relativize: function (element) {
        ...
        element.style.position = "relative";
        var top =
            parseFloat(element.style.top || 0) - (element._originalTop || 0);
        var left =
        ...
        index: function (parentNode, reverse, ofType) {
            ...
            if (reverse) {
                for (
                    ...
                    if (
                        node.nodeType == 1 &&
                        (!ofType || node._countedByPrototype)
                    )
                        node.nodeIndex = j++;
                }
            } else {
...
Form.Element.Serializers = {
    ...
    select: function (element, index) {
        if (Object.isUndefined(index))
            return this[
                element.type == "select-one" ? "selectOne" : "selectMany"
            ](element);
        else {
            var opt,
                value,
...
Object.extend(
    ...
    (function () {
        ...
        function destroyWrapper(id, eventName, handler) {
            ...
            c[eventName] = c[eventName].without(
        ...
        return {
            ...
            stopObserving: function (element, eventName, handler) {
                ...
                if (!handler && eventName) {
                    getWrappersForEventName(id, eventName).each(function (
                        wrapper
                    ) {
